...
```
CPU 告警: 登录目标服务器 , 查找占用 CPU 资源的进程/服务; 
如果是业务进程 , 找到对应项目负责人告知情况 , 寻求处理;
如果是中间件进程 , 查看对应中间件日志 , 网卡流量等来确定是哪个进程调用引起的异常; 暂时找不到原因的 , 与各开发主管沟通确认是否可以重启中间件.
如果是系统进程 / 陌生进程 , 检查系统日志等来确认是否遭到入侵, 确认遭到入侵则切换到备用服务器 , 对该服务器进行下线并查找原因进行处理 . 找不到原因的话 , 重装系统并更新补丁后 , 重新发布上线.
```

```
内存告警: 登录目标服务器 , 查找占用内存资源的进程/服务.
如果是业务进程 , 找对应项目负责人告知情况 , 寻求处理; 
如果是中间件进程 , 查看对应中间件日志寻找原因 , 暂时找不到原因的 , 与各项目负责人沟通确认是否可以重启中间件.
如果是系统进程 / 陌生进程 , 检查系统日志等来确认是否遭到入侵 , 确认遭到入侵则切换到备用服务器 , 对该服务器进行下线并查找原因进行处理 . 找不到原因的话 , 重装系统并更新补丁后 , 重新发布上线.
```

```
磁盘告警: 登录目标服务器 , 查看占用资盘资源的文件位置.
如果是业务进程 log 文件, 在对应项目负责人确认可以删除日志的情况下 , 删除老日志. 如果日志没有切割 , 则尝试使用 Logrotate 对日志进行切割 , 如果不能切割 , 则 echo 空字符串到日志文件.
如果是非业务进程 Log 文件 , 确认文件用途 , 是否可以删除.
```

```
网卡流量告警: 登录目标服务器 , 查找占用网卡流量的进程/服务.
如果是业务进程 , 找对应项目负责人告知情况 , 寻求处理.
如果是中间件进程 , 查找网卡流量流入/流出的目的地址 , 如果是业务进程导致 , 找对应项目负责人寻求处理 , 如果非业务进程导致 , 需要检查系统日志等确定是否遭到入侵. 并确定是否需要切换到备用服务器.
```
...
```
容器 CPU 告警: 找对应项目负责人告知情况进行处理 , 并审视告警规则中阈值设定是否合理 , 如果不合理则调整告警阈值 , 如果影响宿主机内其他容器正常运行 , 则对该容器的 CPU 资源进行限制.
```

```
容器内存告警: 找对应项目负责人告知情况进行处理 , 并审视告警规则中阈值设定是否合理 , 如果不合理则调整告警阈值 , 如果影响宿主机内其他容器正常运行 , 则对该容器的内存资源进行限制.
```
...
```
JVM heap 内存告警: 找对应项目负责人告知情况进行处理 , 如果告警规则阈值不合理则调整阈值 , 如果确实需要更多内存 , 则在启动 jar 包的命令中增加 Xmx 的内存大小.
```
...
```
登录对应业务所在服务器 , 检查 Nginx 及对应的后端服务是否在线并尝试重启服务 , 尽快恢复业务上线. 然后联系对应项目负责人 , 提供告警时刻日志及监控情况以便开发分析.
```



告警处理完后 , 在钉钉告警群中告知问题及解决方案.
...
| :--------: | :-------------------: | :-------------------: | :-------------------: | :-------------------: | :-------------------: | :-------------------: | :-------------------: |
|  告警检查  |        史家远         |        史家远         |        史家远         |        史家远         |        史家远         |        史家远         |        史家远         |
|  告警处理  | 史家远/对应项目负责人 | 史家远/对应项目负责人 | 史家远/对应项目负责人 | 史家远/对应项目负责人 | 史家远/对应项目负责人 | 史家远/对应项目负责人 | 史家远/对应项目负责人 |
| 备选联系人 |     项目后端主管      |     项目后端主管      |     项目后端主管      |     项目后端主管      |     项目后端主管      |     项目后端主管      |     项目后端主管      |
| 紧急联系人 |        束先锋         |        束先锋         |        束先锋         |        束先锋         |        束先锋         |        束先锋         |        束先锋         |
